...
def get_gt_points_transform(
    ...
    mesh.vertices = mesh.vertices * scale + set_location

    save_ply = f"{save_dir}/{scan_name}.ply"
    mesh.export(save_ply)
...
def bezier_derivative(P0, P1, P2, P3, t):
    """Computes the derivative B'(t) of the cubic Bézier curve at t."""
    return (3*(1-t)**2 * (P1 - P0) + 
            6*(1-t)*t * (P2 - P1) + 
            3*t**2 * (P3 - P2))

def bezier_derivative_array(P0, P1, P2, P3, t):
    """Computes the derivative B'(t) of the cubic Bézier curve at t."""
    return (3*(1-t)**2 * (P1 - P0) + 
            6*(1-t)*t * (P2 - P1) + 
            3*t**2 * (P3 - P2))

def bezier_length_simpson_fast(P0, P1, P2, P3, n=100):
    """Computes the length of a 3D Bézier curve using Composite Simpson's Rule."""
    if n % 2 == 1:  # Ensure n is even
        n += 1
    
    t_values = np.linspace(0, 1, n+1)  # Evenly spaced t values
    h = 1 / n
    
    # Compute |B'(t)| at sampled points
    # f_values = np.linalg.norm([bezier_derivative(P0, P1, P2, P3, t) for t in t_values], axis=1)
    f_values = np.linalg.norm(bezier_derivative_array(P0[None, :], P1[None, :], P2[None, :], P3[None, :], t_values[:, None]), axis=1)
    
    # Apply Simpson's Rule: h/3 * [f0 + 4(f1+f3+...) + 2(f2+f4+...) + fn]
...
def bezier_length_simpson_fast_torch(P0, P1, P2, P3, n=100):
    """Computes the length of a 3D Bézier curve using Composite Simpson's Rule."""
    if n % 2 == 1:  # Ensure n is even
        n += 1
    
    t_values = torch.linspace(0, 1, n+1, device=P0.device)  # Evenly spaced t values
    h = 1 / n
    
    # Compute |B'(t)| at sampled points
    # f_values = np.linalg.norm([bezier_derivative(P0, P1, P2, P3, t) for t in t_values], axis=1)
    f_values = torch.norm(bezier_derivative_array(P0[None, :], P1[None, :], P2[None, :], P3[None, :], t_values[:, None]), dim=1)
    
    # Apply Simpson's Rule: h/3 * [f0 + 4(f1+f3+...) + 2(f2+f4+...) + fn]
    integral = (h/3) * (f_values[0] + 
                        4 * torch.sum(f_values[1:n:2]) + 
                        2 * torch.sum(f_values[2:n-1:2]) + 
                        f_values[n])
...
def sample_points_from_curves_diff(
    curves_ctl_pts,
    lines_end_pts,
    sample_resolution=0.005,
):
    ...
    if num_curves > 0:
        ...
        for i in range(num_curves):
            ...
            each_curve = curves_ctl_pts[i, :, :].reshape(4, 3)  # shape: (4, 3)
            
            # import time
            # t0 = time.time()
            # for iii in range(200):
            
            each_curve_np = each_curve.detach().cpu().numpy()
            sample_num = int(
                # bezier_curve_length(each_curve_np, num_samples=100) // sample_resolution
                bezier_length_simpson_fast(each_curve_np[0, :],
                                        each_curve_np[1, :],
                                        each_curve_np[2, :],
                                        each_curve_np[3, :],
            ...
            t = torch.linspace(0, 1, sample_num, device=device)
            matrix_u = torch.stack([t**3, t**2, t, torch.ones_like(t)], dim=0)  # (4, sample_num)

            matrix = torch.matmul(
                torch.matmul(matrix_u.T, matrix_middle), each_curve
            ).reshape(sample_num, 3)

            all_curve_points.append(matrix)

            # Derivative / Direction 
            P0, P1, P2, P3 = each_curve         # (4, 3)
            direction = (t**2)[:, None] * (-3*P0 + 9*P1 - 9*P2 + 3*P3)[None, :] + \
                           (t)[:, None] * (6*P0 - 12*P1 + 6*P2)[None, :] + \
                                          (-3*P0 + 3*P1)[None, :]

            all_curve_directions.append(direction)
            all_stroke_idx.append(torch.ones(sample_num)*i)
    ...
    if num_lines > 0:
        ...
        matrix_middle_l = torch.tensor(np.array([[-1, 1], [1, 0]]), 
            dtype=torch.float32, device=device)

        for i in range(num_lines):
            each_line = lines_end_pts[i, :, :].reshape(2, 3)  # shape: (2, 3)
            sample_num = int(
                torch.norm(each_line[0] - each_line[-1]) // sample_resolution
            )
            t = torch.linspace(0, 1, sample_num, device=device)

            matrix_u_l = torch.stack([t, torch.ones_like(t)], dim=0)

            matrix_l = torch.matmul(
                torch.matmul(matrix_u_l.T, matrix_middle_l), each_line
            ).reshape(sample_num, 3)
            all_line_points.append(matrix_l)

            # Calculate the direction vector for the line segment
            direction = each_line[1] - each_line[0]

            all_line_directions.append(torch.ones_like(matrix_l) * direction)

            all_stroke_idx.append(torch.ones(sample_num)*(i+num_curves))
    ...
    if num_curves > 0:
        ...
        all_curve_directions = all_curve_directions / (torch.norm(all_curve_directions, dim=1, keepdim=True) + 1e-6)
    else:
        all_curve_points = torch.tensor([], device=device)
        all_curve_directions = torch.tensor([], device=device)
    ...
    if num_lines > 0:
        ...
        all_line_directions = all_line_directions / (torch.norm(all_line_directions, dim=1, keepdim=True) + 1e-6)
    else:
        all_line_points = torch.tensor([], device=device)
        all_line_directions = torch.tensor([], device=device)

    all_stroke_idx = torch.cat(all_stroke_idx).long()
