...
def get_paths_from_data_config(data_config, scene_name):
    ...
    if data_config["parser_type"] == "emap":
        ...
        rgb_dir = data_dir /  "color"

        # return strings
        return images_dir.as_posix(), cameras_path.as_posix(), seed_points_ply_path.as_posix(), rgb_dir.as_posix()
        ...
        return images_dir.as_posix(), colmap_base_dir_str, sparse_points_path_str, rgb_dir.as_posix()
    

def parse_data(data_config, scene_name):
    images_dir, data_input_path, seed_points_path, rgb_dir = get_paths_from_data_config(data_config, scene_name)
    print(data_config)

    if data_config["parser_type"] == "colmap":
        print(f"Images dir : {images_dir}")
        print(f"RGBs dir : {rgb_dir}")
    ...
    dataparser = DataParserFactory.get_parser(data_config["parser_type"],
                                                   data_input_path,
                                                   dp_kwargs)
    return dataparser, images_dir, seed_points_path, rgb_dir