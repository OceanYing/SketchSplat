...
from sketchgaussians.data.dataparsers import DataParser
...
class InputDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, Tensor]:
        ...
        image = self.views[idx]['image']
        view = {'image':image, 'idx':torch.tensor(idx, dtype=torch.int64)}
        if "rgb" in self.views[idx].keys():
            view["rgb"] = self.views[idx]["rgb"]
        return view