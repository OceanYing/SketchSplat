...
def main():
    ...
    if args.sub_exp_name is None:
        output_dir = os.path.join(output_base, exp_name, args.scene_name)
    else:
        output_dir = os.path.join(output_base, exp_name, args.scene_name + "_" + args.sub_exp_name)
    ...
    fit_out_dir = os.path.join(output_dir, "fitting")
    os.makedirs(fit_out_dir, exist_ok=True)

    if args.output_json is None:
        output_json = os.path.join(fit_out_dir, 'parametric_edges.json')
    else:
    ...
    io_utils.write_gaussian_params_as_ply(pos, scales, quats, opacities, os.path.join(fit_out_dir, 'gaussians_all.ply'))
    ...
    if args.save_filtered:
        ...
        io_utils.write_gaussian_params_as_ply(pos, scales, quats, opacities, os.path.join(fit_out_dir, 'gaussians_filtered.ply'))
        
        # save pts with major dirs for easy visualization
        io_utils.write_pts_with_major_dirs_as_ply(pos, major_dirs, os.path.join(fit_out_dir, 'pts_with_major_dirs.ply')) 
    ...
    cnum = len(valid_clusters)
    cmap = np.random.randint(64, 256, [cnum, 3])
    colors = np.zeros_like(points)
    for (i,c) in enumerate(valid_clusters):
        colors[list(c), :] = cmap[i, :]

    io_utils.write_pts_with_rgb(os.path.join(fit_out_dir, 'cluster_points.ply'), points, colors)
    ...
    if args.save_sampled_points:
        ...
        o3d.io.write_point_cloud(os.path.join(fit_out_dir, f'edge_sampled_points_{args.sample_resolution}.ply'), pcd)
