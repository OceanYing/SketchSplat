...
def main(metrics_pr):
    ...
    for scan_name in tqdm(scan_names):
        our_pts_ply_file = os.path.join(output_base_dir, scan_name+suffix, "gaussians_all.ply")
    ...
    for scan_name in tqdm(scan_names):
        ...
        if pts is not None:
            ...
            metrics[scan_name]["sketchgaussians"] = {"chamfer_dist": chamfer_dist, "acc": acc, "comp": comp}
            # print("sketchgaussians")
    ...
    for key in metrics.keys():
        if "sketchgaussians" in metrics[key].keys():
            ours = metrics[key]["sketchgaussians"]
