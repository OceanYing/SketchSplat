...
def main(metrics_pr):
    ...
    parser.add_argument("--version", type=str, default="release", help="Version of our model")
    ...
    output_base_dir = f"output/{args.dataset}/{version}_{edge_detector}"

    if scan_names == "all":
        # scan_names = list(os.listdir(output_base_dir))
        if dataset_name == "ABC":
            scan_names = list(os.listdir(os.path.join(gt_base_dir, "sampled_pts")))
        elif dataset_name == "DTU":
            scan_names = list(os.listdir(os.path.join(gt_base_dir, "edge_points")))

        scan_names = [s.split('_')[0] for s in scan_names]
    
    else:
        scan_names = scan_names.split(",")
    
    if args.sub_exp_name != None:
        suffix = "_" + args.sub_exp_name
    else:
        suffix = ""
    
    scene_number = len(scan_names)
    for scan_name in tqdm(scan_names):
        our_pts_ply_file = os.path.join(output_base_dir, scan_name+suffix, "fitting/parametric_edges.json")
        if not os.path.exists(our_pts_ply_file):
            print("ply file not exist: ", our_pts_ply_file)
            exit()
    ...
    for scan_name in tqdm(scan_names):
        ...
        ply_path_dtu = os.path.join(gt_base_dir, "edge_points", f"{scan_name}/edge_points.ply")
        if os.path.exists(ply_path):
            pcd_gt = o3d.io.read_point_cloud(ply_path)
            gt_points = np.asarray(pcd_gt.points)
        
        elif os.path.exists(ply_path_dtu):
            pcd_gt = o3d.io.read_point_cloud(ply_path_dtu)
            gt_points = np.asarray(pcd_gt.points)
        ...
        if args.sub_exp_name != None:
            scan_name_full = scan_name + "_" + args.sub_exp_name
        else:
            scan_name_full = scan_name
            
        if use_filtered_points:
            our_pts_ply_file = os.path.join(output_base_dir, scan_name_full, "fitting", "gaussians_filtered.ply")
            print(our_pts_ply_file)
            # exit()
            try:
                # our_pts_ply_file = os.path.join(output_base_dir, scan_name_full, "fitting", "gaussians_filtered.ply")
                # our_pts_ply_file = os.path.join(output_base_dir, scan_name_full, "fitting", "gaussians_all.ply")
            ...
            try:
                ...
                our_pts_ply_file = os.path.join(output_base_dir, scan_name_full, "fitting", f"edge_sampled_points_{args.sample_resolution}.ply")
                ...
                if os.path.exists(our_pts_ply_file):
                    ...
                    our_parametric_edges_file = os.path.join(output_base_dir, scan_name_full, "parametric_edges.json")
    ...
    for key in metrics_pr.keys():
        if len(metrics_pr[key]) == 0:
            ...
            print(f"{key}: {mean_val} ({np.array(metrics_pr[key]).size}/{scene_number})")
    ...
    for key in vals.keys():
        ...
        print(f"{key}: {mean_val} ({np.array(vals[key]).size}/{scene_number})")
